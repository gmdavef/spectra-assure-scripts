...
- **consolidate_components.py** - Consolidates identical components with different `location` into a single component with multiple `occurrences`. Works on a CycloneDX BOM (v1.6 or 1.7, JSON format). Use the JSON file(s) in test_files folder to see it in action.
